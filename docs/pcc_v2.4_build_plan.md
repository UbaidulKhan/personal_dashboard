...
## 8. Pillar F: Knowledge & Notes (Module 25) - INTEGRATED
* **The Second Brain:** Built-in Markdown editor with **Obsidian-style** `[[bi-directional links]]` and `![[embeds]]`.
* **UI/UX:** Google Keep-style masonry grid with pinning and global PostgreSQL full-text search (`tsvector`).
* **Deep Integration:** "Related Notes" sidebar appears in all other pillars (e.g., viewing a stock shows related research notes).
* **Mentions:** Integrated `@` mention system to link notes directly to entities (e.g., `@Stock:AAPL`, `@Ticket:12345`, `@Salah:Fajr`).
* **Editable Embeds:** The Markdown editor component allows editing a note from any module (e.g., edit a work note inside the TDX ticket view).

---

## 9. Pillar G: Strategic Planning (Module 26) - INTEGRATED
* **The Daily Note:** Auto-generated at 6:00 AM; embeds the Morning Routine (Module 24) narrative and today's prayer times (Module 16).
* **Master View:** Unified task list aggregating Family, Side Hustle, and Work tasks into one executive dashboard.
* **Conflict Detection:** Visual flags for overlapping professional deadlines and family activities on a unified timeline.

---

## 10. Pillar H: Work Management (Modules 27–29) - INTEGRATED
* **Module 27 (TDX Sync):** Automated pull of assigned tickets from TeamDynamix. Maps `DueDate` to the unified calendar.
* **Automated Cleanup:** Tickets marked "Resolved" or "Closed" in TDX are automatically removed from the PCC database to maintain a clean environment.
* **Module 28 (Work-Knowledge Base):** Notes tagged `[work]` are isolated to professional dashboard views.
* **Module 29 (Linear Backup):** Nightly (10:00 PM) text-based calendar mirror pushed to Google Keep.
    * *Format:* Date -> Separator -> Time -> Task Description.
* **Quiet Hours:** Suppression of professional push notifications from 6:00 PM to 6:00 AM.

---

## 11. Final Implementation Checklist (Cursor AI)
1. **Schema Update:** Implement the PostgreSQL junction table for bi-directional linking and entity mentions.
2. **Editor Build:** Construct the Markdown editor component supporting links, embeds, and cross-module editing.
3. **Sync Engine:** Deploy the TDX API background worker and the Google Keep Linear Backup script.
4. **Planning Dashboard:** Build the "Daily Note" auto-generator and the unified task aggregator.
